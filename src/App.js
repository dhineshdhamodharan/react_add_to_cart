...
function App() {
  const [products,setProduct]=useState(
    ...
    //Disable the "Add to cart" button when item is added to cart. If the item is removed from cart enable the "Add to cart button"
    let disableAddtoCart=(id)=>{
      let product=products.find(obj=>obj.id==id);
          ...
          <div className="col-lg-8">
            <div className="row">
              ...
              products.map((product) =>{
                return <Card data={product} handleCart={addtoCart} disabled={disableAddtoCart}/>
              })
            }
            </div>
          </div>
